    ...
    public sealed class ModelComparison
    {
        ...
        public ModelComparison(Model refModel, Model checkModel)
        {
            this.entities_not_in_check = refModel.entities
                .Where(re => !checkModel.entities.Any(ce => IEntityKey.Comparer.Equals(ce, re)))
                .ToList();
            this.entities_not_in_ref = checkModel.entities
                .Where(ce => !refModel.entities.Any(re => IEntityKey.Comparer.Equals(re, ce)))
                .ToList();

            this.entities_different_properties = refModel.entities
                .Select(re => new PropDiff() { @ref = re, check = checkModel.entities.FirstOrDefault(ce => IEntityKey.Comparer.Equals(ce, re))! })
